...
SDL_Surface *OIMG_LoadJPG_RW(SDL_RWops *src)
{
	int start;
	...
	if(setjmp(jerr.escape)) {
		...
		SDL_RWseek(src, start, RW_SEEK_SET);
		OIMG_SetError("JPEG loading error");
		return NULL;
	}
...
int OIMG_InitJPG()
{
	OIMG_SetError("JPEG images are not supported");
	return(-1);
}
